...
const playerOnRooms = (sender, from, data) => {
    let result = findObject(data, "id", sender)
    let index = false
    if (result.length > 0 && result[0] === from) {
        index = true
    } else {
        return index
    }
...
const pagii = (data) => {
    if (data.dead.length < 1) {
        return `Mentari telah terbit, tidak ada korban berjatuhan malam ini, warga kembali melakukan aktifitasnya seperti biasa.\n60 detik tersisa sebelum waktu penentuan, para warga dipersilahkan untuk berdiskusi\n\n*Hari ke ${data.day}*`
    } else {
        a = ""
        d = ""
        e = []
        ...
        textnya = `Pagi telah tiba, warga desa menemukan ${data.dead.length > 1 ? "beberapa" : "1"} mayat di tumpukan puing dan darah berceceran. ${a ? a + " telah mati! " : ""}${d.length > 1 ? ` ${d} hampir dibunuh, namun *Guardian Angel* berhasil melindunginya.` : ""}\n\nTak terasa hari sudah siang, matahari tepat di atas kepala, terik panas matahari membuat suasana menjadi riuh, warga desa mempunyai 60 detik untuk berdiskusi\n\n*Hari ke ${data.day}*`
        return textnya
    }
}
...
async function pagi(conn, x, data) {
    ...
    return await conn.sendMessage(x.room, { text: pagii(x), mentions: ment })
}
...
async function voting(conn, x, data) {
    ...
    textnya = "Senja telah tiba. Seluruh warga berkumpul di balai desa untuk memilih siapa yang akan dieksekusi. Sebagian warga terlihat sibuk menyiapkan alat penyiksaan untuk malam ini. Kalian mempunyai waktu selama 60 detik untuk memilih! Hati-hati, ada penghianat diantara kalian!\n\n*Player :*\n"
    shortPlayer(x.room, data)
    for (let i = 0; i < x.player.length; i++) {
        textnya += `*${x.player[i].number}.* @${x.player[i].id.replace("@s.whatsapp.net", "")} ${x.player[i].isdead === true ? "- *dead*" : "- *alive*"}\n`
        ment.push(x.player[i].id)
    }
    ...
    return await conn.sendMessage(x.room, { text: textnya, mentions: ment })
}

async function malam(conn, x, data) {
    var hasil_vote = voteResult(x.room, data)
    if (hasil_vote === 0) {
        textnya = `Terlalu bimbang menentukan pilihan. Warga pun pulang ke rumah masing-masing, tidak ada yang dieksekusi hari ini. Bulan bersinar terang, malam yang mencekam telah datang. Semoga tidak ada yang mati malam ini. Pemain malam hari: kalian punya 60 detik untuk beraksi!`
        return conn.sendMessage(x.room, { text: textnya }).then(() => {
            changeDay(x.room, data)
        ...
        if (hasil_vote.role === "werewolf") {
            textnya = `Warga desa telah memilih dan sepakat @${hasil_vote.id.replace("@s.whatsapp.net", "")} dieksekusi mati.\n\n@${hasil_vote.id.replace("@s.whatsapp.net", "")} adalah ${hasil_vote.role} ${emoji_role(hasil_vote.role)}`
            voteKill(x.room, data)
            let ment = []
            ment.push(hasil_vote.id)
            return await conn.sendMessage(x.room, { text: textnya, mentions: ment }).then(() => {
                changeDay(x.room, data)
                voteDone(x.room, data)
                resetVote(x.room, data)
                clearAllVote(x.room, data)
...
async function skill(conn, x, data) {
    ...
    if (getWinner(x.room, data).status != null || x.win != null) {
        ...
        shortPlayer(x.room, data)
        for (let i = 0; i < x.player.length; i++) {
            tok1 += `*${x.player[i].number}.* @${x.player[i].id.replace("@s.whatsapp.net", "")}${x.player[i].isdead === true ? " - *dead*" : " - *alive*"}\n`
            membernya.push(x.player[i].id)
        }
        for (let i = 0; i < x.player.length; i++) {
            tok2 += `*${x.player[i].number}.* @${x.player[i].id.replace("@s.whatsapp.net", "")} ${x.player[i].role === "werewolf" || x.player[i].role === "sorcerer" ? `${x.player[i].isdead === true ? ` - *dead*` : ` ${x.player[i].role}`}` : " - *alive*"}\n`
            membernya.push(x.player[i].id)
        }
        for (let i = 0; i < x.player.length; i++) {
...
async function win(x, t, conn, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await conn.sendMessage(sesinya, { text: textnya, mentions: ment }).then(() => {
            delete data[x.room]
        })
    } else if (getWinner(x.room, data).status === true) {
        textnya = `*TEAM - WARGA - WIN*\n\n`
        let ment = []
        for (let i = 0; i < x.player.length; i++) {
            if (
